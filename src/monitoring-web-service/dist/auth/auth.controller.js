...
class AuthController {
    ...
    static refreshToken = (0, error_middleware_1.asyncHandler)(async (req, res) => {
        ...
        const newTokenPair = jwt_service_1.JwtService.generateTokenPair(user.id, user.tenantId, user.email, user.role, user.permissions);
        refreshTokenStore.delete(payload.tokenId);
        ...
        logger_1.loggers.auth.tokenRefresh(user.id, user.tenantId, {
            requestId: req.requestId,
            ip: req.ip,
        });
        res.json({
