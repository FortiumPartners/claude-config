...
const createUserSchema = Joi.object({
  ...
  role: Joi.string().valid('owner', 'admin', 'manager', 'developer', 'viewer').required(),
  password: Joi.string().min(8).max(128).optional()
    .pattern(new RegExp('^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]'))
    .message('Password must contain at least one lowercase letter, one uppercase letter, one digit, and one special character'),
  send_invitation: Joi.boolean().default(true),
  team_assignments: Joi.array().items(
    Joi.object({
...
const updateUserSchema = Joi.object({
  ...
  role: Joi.string().valid('owner', 'admin', 'manager', 'developer', 'viewer').optional(),
...
const inviteUserSchema = Joi.object({
  ...
  expires_in_hours: Joi.number().integer().min(1).max(168).default(72), // Default 3 days
});
...
const bulkUserActionSchema = Joi.object({
  ...
  confirmation: Joi.boolean().when('action', {
    is: 'delete',
...
export function createUserManagementRoutes(db: DatabaseConnection, logger: winston.Logger, authenticateJWT?: (req: AuthRequest, res: express.Response, next: express.NextFunction) => Promise<void>) {
  ...
  const requirePermission = (resource: string, action: string) => {
    return async (req: AuthRequest, res: express.Response, next: express.NextFunction) => {
      try {
        // This would use the JWTService.hasPermission method
        // For now, implementing basic role-based checks
        ...
        // Owner and admin have full user management access
        if (['owner', 'admin'].includes(userRole)) {
          return next();
        }

        // Managers can view and invite users
  ...
  const userManagementLimiter = rateLimit({
    ...
    max: 100, // More generous for management operations
    message: { error: 'Too many user management requests' },
  });

  const invitationLimiter = rateLimit({
    windowMs: 60 * 60 * 1000, // 1 hour
    max: 20, // Limit invitations to prevent spam
    message: { error: 'Too many invitation requests' },
  });

  /**
   * GET /api/users
   * List users in organization with filtering and pagination
   */
  router.get('/', authMiddleware, requirePermission('users', 'read'), async (req: AuthRequest, res) => {
    try {
      ...
      // Main query with user details and team information
      const query = `
   ...
   */
  router.get('/:userId', authMiddleware, requirePermission('users', 'read'), async (req: AuthRequest, res) => {
    try {
   ...
   */
  router.post('/', authMiddleware, requirePermission('users', 'create'), async (req: AuthRequest, res) => {
    try {
      ...
      try {
        ...
        const userResult = await client.query(userQuery, [
          ...
          role,
          passwordHash,
          !!password, // If password provided, consider email verified
        ]);
        ...
        if (team_assignments && team_assignments.length > 0) {
          for (const assignment of team_assignments) {
            await client.query(
              ...
              [req.user!.organization_id, assignment.team_id, newUser.id, assignment.role]
            );
          }
        }

        // TODO: Send invitation email if send_invitation is true and no password provided
        if (send_invitation && !password) {
          // Implementation for email invitation would go here
   ...
   */
  router.put('/:userId', authMiddleware, requirePermission('users', 'update'), async (req: AuthRequest, res) => {
    try {
      const { userId } = req.params;
      ...
      // Prevent role changes that would leave organization without owner
      if (value.role && user.role === 'owner' && value.role !== 'owner') {
        const ownerCount = await db.query(
          'SELECT COUNT(*) as count FROM users WHERE organization_id = $1 AND role = $2 AND is_active = true',
          [req.user!.organization_id, 'owner']
        );
   ...
   */
  router.delete('/:userId', authMiddleware, requirePermission('users', 'delete'), async (req: AuthRequest, res) => {
    try {
      const { userId } = req.params;

      // Check if user exists and get role
      const userResult = await db.query(
        'SELECT id, role, email FROM users WHERE id = $1 AND organization_id = $2',
        [userId, req.user!.organization_id]
      );
      ...
      // Soft delete: deactivate instead of actual deletion to preserve audit trail
      await db.query(
      ...
      // Revoke all tokens
      // This would require the JWTService instance
   ...
   */
  router.post('/invite', invitationLimiter, authMiddleware, requirePermission('users', 'invite'), async (req: AuthRequest, res) => {
    try {
   ...
   */
  router.post('/bulk-action', authMiddleware, requirePermission('users', 'update'), async (req: AuthRequest, res) => {
    try {
      ...
      const usersResult = await db.query(
        'SELECT id, email, role FROM users WHERE id = ANY($1) AND organization_id = $2',
        [user_ids, req.user!.organization_id]
      );
      ...
      // Prevent actions on owners for critical operations
   ...
   */
  router.get('/audit/:userId', authMiddleware, requirePermission('users', 'audit'), async (req: AuthRequest, res) => {
    try {
      const { userId } = req.params;
      const page = parseInt(req.query.page as string) || 1;
      const limit = Math.min(parseInt(req.query.limit as string) || 50, 100);
...
// Note: This would require creating the user_invitations table:
/*
