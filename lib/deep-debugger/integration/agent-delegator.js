...
/**
 * Create AgentDelegator with Claude Code Task tool binding
 *
 * Helper function for agents to create AgentDelegator with proper Task tool integration.
 * This function must be called from within a Claude Code agent context where the Task tool is available.
 *
 * @param {Function} TaskTool - The Task tool function from Claude Code context
 * @param {Object} [options] - Additional options for AgentDelegator
 ...
 * const delegator = createAgentDelegatorWithTaskTool(Task, { defaultTimeout: 900000 });
 *
 * // Delegate GREEN phase
 * const result = await delegator.delegateGreenPhase({ fixTask, tddState, sessionId });
 */
...
class AgentDelegator {
  ...
  async delegate(options) {
    ...
    try {
      // Use Task tool (must be injected by calling agent)
      if (!this.taskTool) {
        throw new Error(
          `AgentDelegator requires Task tool injection. ` +
          `Agent '${agent}' attempted delegation but no taskTool provided.\n\n` +
          ...
          `    }\n` +
          `  });\n\n` +
          `For testing, inject a mock function that returns the expected response format.`
        );
      }

      this.logger(`[Agent Delegation] Delegating to ${agent}...`);
...
module.exports.createAgentDelegatorWithTaskTool = createAgentDelegatorWithTaskTool;
