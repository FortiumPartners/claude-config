...
describe('GreenPhaseDelegator', () => {
  ...
  describe('delegateGreenPhase', () => {
    ...
    beforeEach(() => {
      ...
      // Mock AgentDelegator to return AgentDelegator format
      mockAgentDelegator.delegateGreenPhase.mockResolvedValue(successResponse);
    });
    ...
    it('should successfully delegate to specialist', async () => {
      ...
      expect(mockAgentDelegator.delegateGreenPhase).toHaveBeenCalledTimes(1);
    });
  ...
  describe('Retry Logic', () => {
    ...
    beforeEach(() => {
      ...
      failureResponse = {
        ...
        codeChanges: [{ filePath: 'test.js' }],
        testChanges: [],
        testsPass: false,  // Failing validation
        fixValidation: { testsPass: false },
        implementationTime: 1000
      };
    });

    it('should retry on validation failure', async () => {
      mockAgentDelegator.delegateGreenPhase
      ...
      expect(mockAgentDelegator.delegateGreenPhase).toHaveBeenCalledTimes(2);
      expect(result.attemptNumber).toBe(2);
    });

    it('should exhaust retries after max attempts', async () => {
      mockAgentDelegator.delegateGreenPhase.mockResolvedValue(failureResponse);

      const result = await delegator.delegateGreenPhase(validContext);

      expect(mockAgentDelegator.delegateGreenPhase).toHaveBeenCalledTimes(3); // Initial + 2 retries
  ...
  describe('Error Handling', () => {
    ...
    it('should handle timeout errors', async () => {
      ...
      mockAgentDelegator.delegateGreenPhase.mockRejectedValue(timeoutError);
    ...
    it('should not retry on non-retryable errors', async () => {
      ...
      nonRetryableError.type = 'non-retryable';
      mockAgentDelegator.delegateGreenPhase.mockRejectedValue(nonRetryableError);
  ...
  describe('Edge Cases', () => {
    it('should handle empty code changes array', async () => {
      mockAgentDelegator.delegateGreenPhase.mockResolvedValue({
        success: true,
        fixImplemented: true,
        codeChanges: [],
        testChanges: [],
