 ...
 * 2. Invoke specialist agent via AgentDelegator
 ...
 * - Uses AgentDelegator for specialist agent coordination
 *
...
class GreenPhaseDelegator {
  ...
  /**
   * Invoke specialist agent delegation via AgentDelegator
   *
  ...
  async invokeDelegation(specialist, request) {
    try {
      ...
      const result = await this.agentDelegator.delegateGreenPhase({
        ...
        tddState: {
          currentPhase: request.tddPhase
        },
        sessionId: request.sessionId,
        retryContext: request.retryContext || null
      });

      // Transform AgentDelegator response to expected format for parseSpecialistResponse
      return {
        success: result.success,
        codeChanges: result.codeChanges,
        testChanges: result.testChanges,
        fixValidation: {
          testsPassing: result.testsPass,  // Map testsPass -> testsPassing for workflow compatibility
