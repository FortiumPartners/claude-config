...
class SkillLoader {
  ...
  async _validateCompatibility(frontmatter) {
    if (!frontmatter || !frontmatter.compatible_agents) {
      // No compatibility requirements specified
      return;
    }

    const requiredVersion = frontmatter.compatible_agents[this.agentName];
    if (!requiredVersion) {
      // No requirement for this specific agent
      return;
    }

    if (!this._satisfiesVersionRange(this.agentVersion, requiredVersion)) {
      throw new Error(
        `Skill requires ${this.agentName} ${requiredVersion}, but current version is ${this.agentVersion}`
      );
    }
  }

  /**
   * Internal: Check if version satisfies a semver range
   * @private
   */
  _satisfiesVersionRange(version, range) {
    // Simple semver range checking (supports: >=, >, <=, <, ^, ~, exact)
    ...
    // Handle >= operator
    if (range.startsWith('>=')) {
      const reqParts = range.slice(2).split('.').map(Number);
      for (let i = 0; i < 3; i++) {
        if (versionParts[i] > reqParts[i]) return true;
        if (versionParts[i] < reqParts[i]) return false;
      }
      return true; // Equal
    }
    ...
    // Handle > operator
    if (range.startsWith('>')) {
      const reqParts = range.slice(1).split('.').map(Number);
      for (let i = 0; i < 3; i++) {
        if (versionParts[i] > reqParts[i]) return true;
        if (versionParts[i] < reqParts[i]) return false;
      }
      return false; // Equal doesn't satisfy >
    }

    // Handle ^ operator (compatible with version - allows minor/patch updates)
    if (range.startsWith('^')) {
      const reqParts = range.slice(1).split('.').map(Number);
      if (versionParts[0] !== reqParts[0]) return false; // Major must match
      if (versionParts[0] > 0) return versionParts[1] >= reqParts[1]; // If major > 0, minor can increase
      if (versionParts[1] !== reqParts[1]) return false; // If major is 0, minor must match
      return versionParts[2] >= reqParts[2]; // Patch can increase
    }

    // For other operators, default to false (implement as needed)
    return false;
  }
