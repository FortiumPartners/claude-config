...
async function validateAll() {
  ...
  console.log(`\n🔍 Validating ${yamlFiles.length} YAML agents...\n`);
  ...
  for (const file of yamlFiles.sort()) {
    try {
      ...
      console.log(`✅ ${file.padEnd(45)} - ${data.metadata.name}`);
      results.valid.push({
        file,
      ...
      console.log(`❌ ${file.padEnd(45)} - ERROR`);
  ...
  // Group by category
  const byCategory = results.valid.reduce((acc, agent) => {
  ...
  Object.entries(byCategory).sort().forEach(([category, agents]) => {
    console.log(`   ${category.toUpperCase()} (${agents.length}):`);
